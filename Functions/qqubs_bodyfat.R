qqubs_bodyfat<-function(fit_ubs,nsim,tau){
  fit=list(beta_hat=coef(fit_ubs)[1:(ncol(x)+1)],theta_hat=coef(fit_ubs)[ncol(x)+2],
           sqrt_hes=sqrt(diag(fit_ubs$vcov)))
  theta=fit$theta_hat
  ...
  for(m in 1:nsim){
    ysim[,m]=rubs(n,mu,theta,tau)
    data2=data.frame(y=ysim[,m],data[,2:ncol(data)])
    fit=unitquantreg(y~x1+x2+x3+x4+x5,family= 'ubs', link='logit', link.theta='identity',
                     tau=tau, data=data2)
    parameters1=coef(fit)
    res_sim[,m]=-log(1-pubs_d(parameters1,ysim[,m],x,tau))
  }
  eta=-log(1-pubs_d(parameters0,y,x,tau))
