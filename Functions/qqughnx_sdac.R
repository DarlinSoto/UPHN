qqughnx_sdac<-function(fit_ughnx,nsim,tau){
  fit=list(beta_hat=coef(fit_ughnx)[1:(ncol(x)+1)],theta_hat=coef(fit_ughnx)[ncol(x)+2],
           sqrt_hes=sqrt(diag(fit_ughnx$vcov)))
  theta=fit$theta_hat
  ...
  for(m in 1:nsim){
    ...
    res_sim[,m]=-log(1-pughnx_d(parameters1,ysim[,m],x,tau))
  }
  
  index1=apply(res_sim,2,sum)>200
  index2=apply(res_sim,2,sum)<300
  index=which((index1+index2)==2)
  res_sim=res_sim[,index]
  eta=-log(1-pughnx_d(parameters0,y,x,tau))
  ...
  title(main='ughnx', adj = 0, line = 0.3)
  lines(x = res_teo, y = res_lwr)
  lines(x = res_teo, y = res_upr)
  lines(x = res_teo, y = res_mid, lty = 2)
