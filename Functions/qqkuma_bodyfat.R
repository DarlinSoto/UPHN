qqkuma_bodyfat<-function(fit_kum,nsim,tau){
  fit=list(beta_hat=coef(fit_kum)[1:(ncol(x)+1)],theta_hat=coef(fit_kum)[ncol(x)+2],
           sqrt_hes=sqrt(diag(fit_kum$vcov)))
  theta=fit$theta_hat
  ...
  for(m in 1:nsim){
    ysim[,m]=rkum(n,mu,theta,tau)
    #ysim[,m]=ysim[,m]-0.001
    #ysim[which(ysim[,m]<0),m]=0.001
    data2=data.frame(y=ysim[,m],data[,2:ncol(data)])
    fit=unitquantreg(y~x1+x2+x3+x4+x5,
                     family= 'kum', link='logit', link.theta='identity',
                     tau=tau, data=data2)
    
    parameters1=coef(fit)
    res_sim[,m]=-log(1-pkuma(parameters1,ysim[,m],x,tau))
  }
  eta=-log(1-pkuma(parameters0,y,x,tau))
