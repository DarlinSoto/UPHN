qqugompertz_bodyfat<-function(fit_ugompertz,nsim,tau){
  fit=list(beta_hat=coef(fit_ugompertz)[1:(ncol(x)+1)],theta_hat=coef(fit_ugompertz)[ncol(x)+2],
           sqrt_hes=sqrt(diag(fit_ugompertz$vcov)))
  theta=fit$theta_hat
  ...
  for(m in 1:nsim){
    ysim[,m]=rugompertz(n,mu,theta,tau)
    data2=data.frame(y=ysim[,m],data[,2:ncol(data)])
    fit=unitquantreg(y~x1+x2+x3+x4+x5,family= 'ugompertz', link='logit', link.theta='identity',
                     tau=tau, data=data2)
    parameters1=coef(fit)
    res_sim[,m]=-log(1-pugompertz_d(parameters1,ysim[,m],x,tau))
  }
  eta=-log(1-pugompertz_d(parameters0,y,x,tau))
  ...
  title(main='ugompertz', adj = 0, line = 0.3)
  lines(x = res_teo, y = res_lwr)
  lines(x = res_teo, y = res_upr)
  lines(x = res_teo, y = res_mid, lty = 2)
