qquweibull_bodyfat<-function(fit_uweibull,nsim,tau){
  ...
  for(m in 1:nsim){
    ...
    fit=unitquantreg(y~x1+x2+x3+x4+x5,family= 'uweibull', link='logit', link.theta='identity',
                     tau=tau, data=data2)
  ...
  title(main='uweibull', adj = 0, line = 0.3)
  lines(x = res_teo, y = res_lwr)
  lines(x = res_teo, y = res_upr)
  lines(x = res_teo, y = res_mid, lty = 2)
